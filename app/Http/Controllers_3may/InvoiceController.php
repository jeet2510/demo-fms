...
class InvoiceController extends Controller
{
    public function invoiceStore(Request $request)
    {
        ...
        foreach ($request->file('hand_over') as $file) {
            ...
            $handOverFileName = time() . '_hand_over_'.$validatedData['booking_id'].'.'.$file->getClientOriginalExtension();
        ...
        foreach ($request->file('border_receipt') as $file) {
            ...
            $borderReceiptFileName = time() . '_border_receipt_'.$validatedData['booking_id'].'.'.$file->getClientOriginalExtension();
    ...
    public function show($id)
    {
        ...
        // dd($invoice, $id);
      $booking_id = Booking::where('booking_id', $id)->first();
        $paidAmounts = Transaction::where('booking_id', $booking_id->id)
            ->pluck('paid_amount', 'driver_id');
            // dd($paidAmounts);
        return view('invoices.show', compact('invoice', 'paidAmounts'));
    }
