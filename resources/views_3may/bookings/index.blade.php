            ...
            <tbody>
                ...
                @foreach ($bookings as $index => $booking)
                    <tr>
                        ...
                        @if($latestTransaction)
                            @php
                                $semiTotalAmounts = explode(',', $latestTransaction->semi_total_booking_amount);
                                $paidAmounts = explode(',', $latestTransaction->paid_amount);
                                $balanceAmounts = [];

                                $count = count($semiTotalAmounts) < count($paidAmounts) ? count($semiTotalAmounts) : count($paidAmounts);

                                for ($i = 0; $i < $count; $i++) {
                                    $semiTotal = (float) $semiTotalAmounts[$i];
                                    $paid = (float) $paidAmounts[$i];
                                    $balanceAmounts[] = $semiTotal - $paid;
                                }

                                $totalBalanceAmount = array_sum($balanceAmounts);
                            @endphp
                            {{-- Total Balance Amount: {{ number_format($totalBalanceAmount, 2) }} --}}
                        @endif
                        </td>
                         <td class="p-2">
                            {{ ($booking->total_booking_amount??0) - ($totalAmount ?? 0) }} 
                    ...
                    <td class="p-2">
                        ...
                        <form action="{{ route('bookings.destroy', $booking->id) }}" method="post" class="inline" style="display: inline-block;">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="btn btn-danger" style="width: 50px;" disabled>Delete</button>
                        </form>
                        @endif
                        <a href="{{ route('bookings.show', $booking->booking_id) }}" class="btn btn-primary"
